...
// Workers 
export const getStaff = async ({params}: any) => axios.get("leads", {params}).then(res => res.data).catch(err => err)
export const editStaff = async ({worker, id}: any) => axios.put(`workers/${id}`, worker).then(res => res.data).catch(err => err)
